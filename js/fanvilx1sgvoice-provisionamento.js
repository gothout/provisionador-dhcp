            // Função para Fanvil
            function provisionFanvilX1SGvoice() {
                // Mostre o modal específico para Fanvil
                $('#fanvilX1SGvoiceModal').modal('show'); // Certifique-se de que este ID corresponde ao ID do seu modal no HTML

                // Aqui, você pode adicionar lógicas de interface, como iniciar um spinner ou indicador de carregamento
                $('#fanvilX1SGvoiceProvisioningStatus').html('<div>Carregando...</div>'); // Você pode substituir isso por um ícone de carregamento ou outra coisa

                // Inicia o pedido AJAX
                $.ajax({
                    type: 'POST',
                    url: '/php-scrp/provisionador/fanvilx1sgvoice.php', // Chamada para o script específico de Fanvil
                    ...
                    success: function(response) {
                        ...
                        setTimeout(function() {
                            ...
                            $('#fanvilX1SGvoiceProvisioningStatus').html(response); // Mostra a resposta do backend
                    ...
                    error: function() {
                        ...
                        $('#fanvilX1SGvoiceProvisioningStatus').text('Erro no processo. Tente novamente.');
